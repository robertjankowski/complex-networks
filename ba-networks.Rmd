...
clustering <- read.csv("output/ba_simulation_par_2_clustering.txt", header = FALSE)
average_path <- read.csv("output/ba_simulation_par_2_avg_path.txt", header = FALSE)
colnames(clustering) <- c("C", "N")
colnames(average_path) <- c("l", "N")
average_path$theory <- log(average_path$N) / log(log(average_path$N))

ggplot(clustering %>% left_join(average_path)) + 
  geom_line(aes(N, theory / 3, color = "ln(N) / ln(ln(N))"), size = 1.1) +
